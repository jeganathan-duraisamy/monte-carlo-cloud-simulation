...
# Get stock data from Yahoo Finance – here, asking for about 10 years of Amazon
    ...
    data = pdr.get_data_yahoo('MSFT', start=decadeAgo, end=today) 
...
# to be generated based on the mean and standard deviation of the recent history
    ...
    profit_loss_days = min(profit_loss_days, len(data) - minhistory)
    response=[]
    for i in range(minhistory, len(data)-profit_loss_days):
        if bs==1: 
